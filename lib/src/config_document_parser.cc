...
namespace hocon { namespace config_document_parser {
    ...
    shared_token parse_context::next_token() {
        ...
        if (_flavor == config_syntax::JSON) {
            if (t->get_token_type() == token_type::UNQUOTED_TEXT && !is_unquoted_whitespace(t)) {
                throw parse_error("Token not allowed in valid JSON: '" + t->token_text() + "'{1}'");
                                                    ...
                                                    path *last_path)
    {
        ...
        if (bad_token == tokens::end_token()->to_string()) {
            ...
            if (!prev_field_name.empty()) {
                part = message + " (if you intended '" + prev_field_name + "' to be part of a value, instead of a key, try adding double quotes around the whole value";
            } else {
                return message;
            }
        } else {
            if (!prev_field_name.empty()) {
                part = message + " (if you intended " + bad_token + " to be part of the value for '" + prev_field_name + "', try enclosing the value in double quotes";
            } else {
                part = message + "(if you intended " + bad_token + "to be part of a key or string value, try enclosing the key or value in double quotes";
            }
        }
    ...
    shared_ptr<config_node_include> parse_context::parse_include(shared_node_list& children) {
        ...
        if (t->get_token_type() == token_type::UNQUOTED_TEXT) {
            ...
            if (kind_text == "url(") {
                ...
                throw parse_error("expecting include parameter to be quoted filename, file(), classpath(), or url(). No spaces are allowed before the open paren. Not expecting: " + t->to_string());
            }
            ...
            if (!tokens::is_value_with_type(t, config_value::type::STRING)) {
                throw parse_error("expecting a quoted string inside file(), classpath(), or url(), rather than {1}" + t->to_string());
            }
            ...
            if (t->get_token_type() == token_type::CLOSE_CURLY) {
                ...
                if (remaining.empty()) {
                    ...
                    if (!emplace.second) {
                        ...
                        if (_flavor == config_syntax::JSON) {
                            throw parse_error("JSON does not allow duplicate fields: '" + key + "' was already seen");
                        }
    ...
    shared_ptr<config_node_complex_value> parse_context::parse_array() {
        ...
        if (next_value) {
            ...
            if (t->get_token_type() == token_type::CLOSE_SQUARE) {
                ...
                throw parse_error("List should have ']' or a first element after the '[', instead had token: " + t->to_string() + " (if you want " + t->to_string() + " to be part of a string value, then double quote it)");
            }
        }
        ...
        while (true) {
            ...
            if (check_element_separator(children)) {
                ...
                if (t->get_token_type() == token_type::CLOSE_SQUARE) {
                    ...
                    throw parse_error("List should have ended with ']' or had a comma, instead had token: " + t->to_string() + " (if you want " + t->to_string() + " to be part of a string value, then double quote it)");
                }
            }
            ...
            if (next_value) {
                ...
                if (is_valid_array_element(t)) {
                    ...
                    throw parse_error("List should have had a new element after a comma, instead had token: " + t->to_string() + " (if you want the comma or " + t->to_string() + " to be part of a string value, then double quote it)");
                }
            }
        }
    }
        ...
        if (t->get_token_type() == token_type::OPEN_CURLY || t->get_token_type() == token_type::OPEN_SQUARE) {
            ...
            if (_flavor == config_syntax::JSON) {
                if (t->get_token_type() == token_type::END) {
                    ...
                    throw parse_error("Document must have an object or array at root, unexpected token: " + t->to_string());
                }
            } else {
    ...
    shared_node_value parse_context::parse_single_value() {
        ...
        if (t->get_token_type() == token_type::IGNORED_WHITESPACE || t->get_token_type() == token_type::NEWLINE ||
            ...
            throw parse_error("The value from with_value_text cannot have leading or trailing newlines, whitespace, or comments");
        }
        if (t->get_token_type() == token_type::END) {
        ...
        if (_flavor == config_syntax::JSON) {
            ...
            if (t->get_token_type() == token_type::END) {
                ...
                throw parse_error("Parsing JSON and the value set in with_value_text was either a concatenation or had trailing whitespace, newlines, or comments");
            }
        } else {
            put_back(t);
