    ...
    shared_value config_delayed_merge_object::attempt_peek_with_partial_resolve(string const& key) const {
        ...
        for (auto&& layer : _stack) {
            if (auto object_layer = dynamic_pointer_cast<const config_object>(layer)) {
                ...
                throw not_resolved_exception("Key '" + key + "' is not available at '" + origin()->description() + "' because value at '" + layer->origin()->description() + "' has not been resolved and may turn out to contain or hide '" + key + "'. Be sure to config::resolve() before using a config object");
